...
token='5721342305:AAF2hf3CdpmSsl4InL6VzxdubAin2FZ_lyk'
...
async def show_weather(message: types.Message):
    await message.answer(text="Добро пожаловать! Введите свой ИИН")

@dp.message_handler()
async def signup(message: types.Message):
    import requests, json

    url = 'http://localhost:8000/main/students/new?student=%s'%message.text
    # new_student = {'student': 'asdasdadsd'}

    # x = requests.post(url, data=json.dumps(new_student))
    x = requests.get(url)

    print("Ответ сервера: ", x.text)
